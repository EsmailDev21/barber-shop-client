...
export default function UserView() {
  const { data, isLoading, error, currentUser } = useAppSelector(selectUsersState);
  const role = useAppSelector(selectAuthState).data.role;
  const finalData =
    role === 'BARBER'
      ? data.filter((u) => u.role === 'CUSTOMER')
      : role === 'ADMIN'
      ? data
      : data.filter((u) => u.role === 'BARBER');
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = finalData.map((n) => n.name);
