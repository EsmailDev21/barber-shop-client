...
import { createReport, selectReportsState } from 'src/redux/slices/ReportsSlice';
...
export default function BarberCard({ data }: BarberCardProps) {
  ...
  const { error, isLoading } = useAppSelector(selectReportsState);
  ...
  return (
    <Box display="flex" justifyContent="center" py={6} mx={3}>
      <Card sx={{ width: 250, textAlign: 'center', boxShadow: 24, p: 2 }}>
        ...
        <CardActions>
          <Stack direction="column" justifyContent="center" spacing={1} mt={2}>
            <Button
              variant="contained"
              color="primary"
              fullWidth
            ...
            <Button variant="outlined" color="error" fullWidth onClick={handleOpenReportModal}>
              Report
            </Button>
          </Stack>
        </CardActions>
      ...
      >
        <Fade in={open}>
          ...
          >
            {barberServices
              ? barberServices.map((service) => <ShopServiceCard service={service} />)
              : null}
          </Box>
        </Fade>
      </Modal>
          ...
          >
            ...
            <form onSubmit={handleReport}>
              <Stack spacing={2}>
                ...
                <TextField
                  name="reason"
                  label="Reason you want to report this user"
